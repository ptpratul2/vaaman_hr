...
def get_data(filters: Filters, attendance_map: dict) -> list[dict]:
    ...
    # Pass the employee_details to get_holiday_map as it now requires it
    holiday_map = get_holiday_map(filters, employee_details)
    data = []
...
def get_attendance_records(filters: Filters) -> list[dict]:
    ...
    print(f"Generated SQL: {query.get_sql()}")  # Debug output
    print(f"Attendance Records: {results}")  # Debug output
    return results
...
def get_employee_related_details(filters: Filters) -> tuple[dict, list]:
    ...
    query = (
        ...
        .left_outer_join(ShiftAssignment).on(ShiftAssignment.employee == Employee.name)
        .left_outer_join(ShiftType).on(ShiftType.name == ShiftAssignment.shift_type)
        .select(
            Employee.name,
            ...
            Employee.branch,
            Employee.company,
            ShiftType.holiday_list.as_("assigned_holiday_list"),  # Fetching holiday list from shift type
        )
        .where(
            (Employee.company == filters.company) &
            (ShiftAssignment.start_date <= getdate()) &
            (ShiftAssignment.end_date >= getdate())
        )
    )
    ...
    employee_details = query.run(as_dict=True); print(employee_details)
...
def get_holiday_map(filters: Filters, employee_details: dict) -> dict[str, list[dict]]:
    holiday_map = frappe._dict()
    Holiday = frappe.qb.DocType("Holiday")

    for employee, details in employee_details.items():
        holiday_lists = [details.holiday_list, details.assigned_holiday_list]  # Consider both employee and shift type holiday lists
        for d in holiday_lists:
            ...
            holiday_map.setdefault(d, []).extend(holidays)  # Append holidays from different lists
